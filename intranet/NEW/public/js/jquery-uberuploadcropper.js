			...
			$.each(imgdata,function(i,obj){
				...
				var str = '<div style="text-align: center;"><img src="'+ options.folder + obj.filename +'?d='+ (new Date()).getTime() +'" class="ubercropimage" id="ubercropimage_'+ i +'" style="margin: 0 auto;" /></div>'+
					...
					'<input type="hidden" name="imgcrop['+ i +'][folder]" id="imgcrop_'+ i +'_folder" value="'+ options.folder +'" class="imgcrop_folder" />';
                                if(options.jcrop.changeH && options.jcrop.changeW) str+='<select style="height:30px;" type="text" value="" name="changeOpt"><option value="w">Width</option><option value="h">Height</option><input style="width:60px;" type="text" value="" name="changeSize" placeholder="Auto">px';
                                else{
                                    if(options.jcrop.changeH)str+='<input type="hidden" id="changeOpt" name="imgcrop['+ i +'][changeOpt]" value="h"><input type="text" value="" name="imgcrop['+ i +'][changeSize]" id="changeSize" placeholder="Auto">px';
                                    else if(options.jcrop.changeW)str+='<input type="hidden" id="changeOpt" name="imgcrop['+ i +'][changeOpt]" value="w"><input type="text" value="" name="imgcrop['+ i +'][changeSize]" id="changeSize" placeholder="Auto">px';
                                    else if(options.jcrop.newW)str+='<input type="hidden" id="changeOpt" name="imgcrop['+ i +'][changeOpt]" value="w"><input type="hidden" value="'+options.jcrop.newW+'" name="imgcrop['+ i +'][changeSize]" id="changeSize" placeholder="Auto">';
                                    else if(options.jcrop.newH)str+='<input type="hidden" id="changeOpt" name="imgcrop['+ i +'][changeOpt]" value="h"><input type="hidden" value="'+options.jcrop.newH+'" name="imgcrop['+ i +'][changeSize]" id="changeSize" placeholder="Auto">';
                                }
