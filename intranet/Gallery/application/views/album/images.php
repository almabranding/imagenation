...
<span class="left w75">
  ...
  <ul id="sortable">
    ...
    <li id="image_<?php echo $image->id; ?>" class="ui-state-default" style="height: <?php echo $config->thumb_height + 10; ?>px">
      ...
      <?php 
       $project=new addThis('http://borndevelopments.com/imagenation/gallery.php?id='.$album->id,'','http://borndevelopments.com/imagenation/intranet/Gallery/uploads/'.$image->name,'share_btn');
       $project->twtext=$project->img;
       $project->getFB();
       $project->getTwitter();
       $project->getPin();
      //echo addThis('http://borndevelopments.com/imagenation/gallery.php?id='.$album->id,$album->name,'http://borndevelopments.com/imagenation/intranet/Gallery/uploads/'.$image->name);
      ?>
      </div>
    </li>
    <?php endforeach; ?>
  ...
  $('#file_upload').uploadify({
    ...
    'onComplete'     : function(event, ID, fileObj, response, data) {
      ...
      $.ajax({
        ...
        error        : function(jqXHR, textStatus, errorThrown) {
          alert('Error occurred when generating thumbnails.'+errorThrown);
        }
      });
    }
  });
